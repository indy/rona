...
icon_load(RonaGl* gl, const char *filename)
{
    int x,y,n;
    GLuint tex;
    unsigned char *data = stbi_load(filename, &x, &y, &n, 0);
    if (!data) RONA_ERROR("[nuklear_example]: failed to load image: %s", filename);
...
static void
device_init(RonaGl* gl, struct device *dev)
{
    GLint status;
...
static void
device_upload_atlas(RonaGl* gl, struct device *dev, const void *image, int width, int height)
{
    gl->genTextures(1, &dev->font_tex);
    gl->bindTexture(GL_TEXTURE_2D, dev->font_tex);
    gl->texParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
    gl->texParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
    gl->texImage2D(GL_TEXTURE_2D, 0, GL_RGBA, (GLsizei)width, (GLsizei)height, 0,
                   GL_RGBA, GL_UNSIGNED_BYTE, image);
}

static void
device_shutdown(RonaGl* gl, struct device *dev)
{
...
static void
device_draw(RonaGl* gl, struct device *dev, struct nk_context *ctx, int width, int height,
